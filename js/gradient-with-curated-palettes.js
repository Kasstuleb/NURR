...
function GradientControls({ tweaks, setTweaks }) {
  ...
  const generateMatchedPalette = () => {
    const count = Math.max(2, Math.min(4, tweaks.colors.length || 4));
    if (GradientPaletteEngine && GradientPaletteEngine.randomMatchedPalette) {
      setTweaks({ colors: GradientPaletteEngine.randomMatchedPalette(count) });
      return;
    }
    const p = WP.PALETTE_PRESETS[Math.floor(Math.random() * WP.PALETTE_PRESETS.length)];
    setTweaks({ colors: p.slice(0, count) });
  };
