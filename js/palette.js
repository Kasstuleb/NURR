...
(function () {
  ...
  function NurrPaletteEditor({ colors, setColors, countLabel, allowAdd=true, minColors=2, maxColors=4, compact=false }) {
    ...
    const randomize = () => {
      const count = Math.max(minColors, Math.min(maxColors, colors.length || maxColors || 4));
      const engine = window.NURR_GRADIENT_PALETTE_ENGINE;
      const p = engine && typeof engine.randomPalette === 'function'
        ? engine.randomPalette(count, { current: colorsRef.current })
        : WP.PALETTE_PRESETS[Math.floor(Math.random() * WP.PALETTE_PRESETS.length)];
      setColors((p || []).slice(0, count));
    };
